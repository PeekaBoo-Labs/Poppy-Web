...
import { WeightType, AIOutput, EffectType, Question, QuestionInput, STI, Tag, ScoreCalculation, calculateScore, getDefaultScore } from "./questions"
...
export default function AIContextProvider({ children }: { children: React.ReactNode }) {
  ...
  const [questionsStack, setQuestionsStack] = useState<Question[]>([new Question_SexualActivity()])
  ...
  const calculateOutput = (): AIOutput => {
    ...
    for (const question of answeredQuestions) {
      ...
      if (question.weightType === WeightType.Additive) {
        ...
        if (question.tags.includes(Tag.Symptom))
